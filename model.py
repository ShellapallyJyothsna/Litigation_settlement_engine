...
# Global model variable
_MODEL = None
MODEL_PKL_PATH = "litigation_model.pkl"
...
# def explain_case(input_df, top_k=5):
#     """
#     Returns SHAP-based explanation for a single prediction.
#     Output format matches existing UI:
#     (feature, display_value, shap_value, description)
#     """
...
#     # Build readable feature impacts
#     impacts = list(zip(feature_names, shap_values[0]))

#     # Group one-hot features back
#     grouped = {}
#     for name, val in impacts:
#         base = name.split("__")[-1].split("_")[0]
#         grouped.setdefault(base, 0)
#         grouped[base] += val

#     # Sort by absolute impact
#     ranked = sorted(grouped.items(), key=lambda x: abs(x[1]), reverse=True)

#     # Format for UI
#     explanations = []
#     for feat, impact in ranked[:top_k]:
#         explanations.append(
#             (feat, "Model-driven", impact, "ML-based contribution")
#         )

#     return explanations
##############strategize case only 
# def explain_case(input_df, action, top_k=5):
#     """
#     Returns SHAP-based explanation with DECISION-AWARE curation.
#     """
...
#     # Convert to list
#     shap_items = [(feat, val) for feat, val in grouped.items()]
...
#         positives = sorted(
#             [x for x in shap_items if x[1] > 0],
#             key=lambda x: x[1],
#             reverse=True
#         )
...
#         # Force conflict narrative
#         selected = positives[:3] + negatives[:2]

#     else:
#         # Default behavior for SETTLE / LITIGATE
#         selected = sorted(
#             shap_items,
#             key=lambda x: abs(x[1]),
#             reverse=True
...
def explain_case(input_df, action, top_k=5):
    ...
    if action == "STRATEGIZE":
    ...
    elif action == "LITIGATE":
        # 🔴 Prefer REAL negative SHAP values (true risk reducers)
        negatives = sorted(
            [x for x in shap_items if x[1] < 0],
            key=lambda x: abs(x[1]),
            reverse=True
        ...
        # Take up to 4 negatives if they exist
        selected = negatives[:4]

        # Fill remaining slots (if needed) with highest absolute positives
        if len(selected) < top_k:
            remaining = top_k - len(selected)
            selected += positives[:remaining]

    else:
        # SETTLE (pure magnitude)
        selected = sorted(
            shap_items,
            key=lambda x: abs(x[1]),
            reverse=True
